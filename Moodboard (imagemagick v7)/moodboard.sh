...
# Final image sizes
FINAL_W=$(((THUMB_W * (COLS + 1)) + (SPACE * (COLS - 1))))
FINAL_H=$(((THUMB_H * (ROWS + 2)) + (SPACE * (ROWS + 1))))
...
montage $(printf "%s\n" *.jpg *.png *.webp 2>/dev/null | shuf -n $TOTAL) \
-tile ${COLS}x${ROWS} \
...
-border 1 \
-alpha on \
png:- | magick - -trim +repage .moodtemp/collage.png

echo "Generating ambient background..."

magick .moodtemp/collage.png \
...
magick .moodtemp/background.png \
